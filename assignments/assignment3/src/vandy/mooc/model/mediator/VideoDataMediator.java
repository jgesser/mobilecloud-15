...
/**
 * Mediates communication between the Video Service and the local storage on the Android device. The methods in this
 * class block, so they should be called from a background thread (e.g., via an AsyncTask).
 */
public class VideoDataMediator {
	 ...
	 */
	private final VideoServiceProxy mVideoServiceProxy;
	...
	public VideoDataMediator() {
		...
		mVideoServiceProxy = new RestAdapter.Builder().setEndpoint(Constants.SERVER_URL).build().create(VideoServiceProxy.class);
	}
			...
			if (videoFile.length() < Constants.MAX_SIZE_MEGA_BYTE) {
				...
				try {
					...
					if (receivedVideo != null) {
						...
						VideoStatus status = mVideoServiceProxy.setVideoData(receivedVideo.getId(), new TypedFile(receivedVideo.getContentType(), videoFile));

						VideoStorageUtils.storeVideoInExternalDirectory(context, videoFile, receivedVideo.getId());
	...
	public String downloadVideo(Context context, long id) {
		try {
			Response data = mVideoServiceProxy.getData(id);
			VideoStorageUtils.storeVideoInExternalDirectory(context, data, id);
			return STATUS_DOWNLOAD_SUCCESSFUL;
		} catch (RetrofitError e) {
			return STATUS_DOWNLOAD_ERROR + e.getMessage();
		}
	}

	public Video rateVideo(long id, int rating) {
		try {
			return mVideoServiceProxy.rateVideo(id, rating);
		} catch (RetrofitError e) {
			e.printStackTrace();
			throw e;
		}
