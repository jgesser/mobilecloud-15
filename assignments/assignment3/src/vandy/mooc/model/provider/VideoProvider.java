...
public class VideoProvider extends ContentProvider {
	...
	/**
	 * Use AcronymDatabaseHelper to manage database creation and version management.
	 */
	private VideoDatabaseHelper mOpenHelper;

	/**
	 * The code that is returned when a URI for more than 1 items is matched against the given components. Must be
	 * positive.
	 */
	private static final int VIDEOS = 100;

	/**
	 * The code that is returned when a URI for exactly 1 item is matched against the given components. Must be
	 * positive.
	 */
	...
	private static UriMatcher buildUriMatcher() {
		...
		// created.
		matcher.addURI(VideoContract.CONTENT_AUTHORITY, VideoContract.PATH_ACRONYM, VIDEOS);
		matcher.addURI(VideoContract.CONTENT_AUTHORITY, VideoContract.PATH_ACRONYM + "/#", VIDEO);
		return matcher;
	}

	/**
	 * Hook method called when Database is created to initialize the Database Helper that provides access to the Video
	 * Database.
	 */
	@Override
	...
	/**
	 * Hook method called to handle requests for the MIME type of the data at the given URI. The returned MIME type
	 * should start with vnd.android.cursor.item for a single item or vnd.android.cursor.dir/ for multiple items.
	 */
	@Override
	...
	/**
	 * Hook method called to handle requests to insert a new row. As a courtesy, notifyChange() is called after
	 * inserting.
	 */
	@Override
	public Uri insert(Uri uri, ContentValues values) {
		...
		switch (sUriMatcher.match(uri)) {
			case VIDEOS:
				// TODOne - replace 0 with code that inserts a row in Table
	...
	public int bulkInsert(Uri uri, ContentValues[] contentValues) {
		...
		switch (sUriMatcher.match(uri)) {
			case VIDEOS:
				...
				try {
					// TODOne -- write the code that inserts all the
					// contentValues into the SQLite database.
					for (ContentValues cv : contentValues) {
						long insert = db.insert(VideoContract.VideoEntry.TABLE_NAME, null, cv);
						if (insert != -1) {
							returnCount++;
						}
					}
	...
	public Cursor query(Uri uri, String[] projection, String selection, String[] selectionArgs, String sortOrder) {
		...
		switch (sUriMatcher.match(uri)) {
			case VIDEOS:
				// TODOne -- replace "null" by writing code to query the
			...
			case VIDEO:
				...
				// from Uri.
				final String rowId = "" + VideoContract.VideoEntry._ID + " = '" + ContentUris.parseId(uri) + "'";

				// TODOne -- replace "null" by writing code to query the
				// SQLite database for the particular rowId based on (a
				// subset of) the parameters passed into the method.
				retCursor = db.query(VideoContract.VideoEntry.TABLE_NAME, projection, rowId, null, null, null, sortOrder);
				break;
			default:
				throw new UnsupportedOperationException("Unknown uri: " + uri);
		}
	...
	/**
	 * Hook method called to handle requests to update one or more rows. The implementation should update all rows
	 * matching the selection to set the columns according to the provided values map. As a courtesy, notifyChange() is
	...
	public int update(Uri uri, ContentValues values, String selection, String[] selectionArgs) {
		...
		switch (sUriMatcher.match(uri)) {
			case VIDEOS:
				...
				// updated.
				// TODOne -- replace "0" with a call to the SQLite database
	...
	/**
	 * Hook method to handle requests to delete one or more rows. The implementation should apply the selection clause
	 * when performing deletion, allowing the operation to affect multiple rows in a directory. As a courtesy,
	 * notifyChange() is called after deleting.
	 */
	@Override
	public int delete(Uri uri, String selection, String[] selectionArgs) {
		...
		switch (sUriMatcher.match(uri)) {
			case VIDEOS:
				// TODOne -- replace "0" with code that deletes the row(s)
